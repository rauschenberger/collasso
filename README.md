...
The [vignette](scripts/vignette.py) contains further examples on multi-task regression with a common feature matrix, multi-task regression with specific feature matrices, and multi-task regression with privileged information. This repository also contain the scripts for a [simulation](scripts/simulation.py) and an [application](scripts/application.py).
